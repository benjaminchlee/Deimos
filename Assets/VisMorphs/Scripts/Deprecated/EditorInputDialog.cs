...
namespace DxR.VisMorphs.Deprecated
{
    ...
    public class EditorInputDialog : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            UnityEngine.GUI.SetNextControlName("inText");
            inputText = EditorGUILayout.TextArea(inputText);
            UnityEngine.GUI.FocusControl("inText");   // Focus text field
            EditorGUILayout.Space(12);

            // Draw OK / Cancel buttons
            var r = EditorGUILayout.GetControlRect();
            //r.width /= 2;
            if (UnityEngine.GUI.Button(r, okButton))
            {
                onOKButton?.Invoke();
                shouldClose = true;
            }
            ...
            r.x += r.width;
            if (UnityEngine.GUI.Button(r, cancelButton))
            {
